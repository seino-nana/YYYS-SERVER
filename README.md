...
| 参数名   | 必选 | 类型   | 说明                          |
| :------- | :--- | :----- | ----------------------------- |
| category | 是   | string | 二级分类(动作、科幻、喜剧...) |
| num      | 是   | number | 5                             |
| page     | 是   | number | 1                             |

调用例子：`/banners?category=动作C&num=5&page=1`
...
调用例子：`/category/deep?type=1&num=5&page=1`
...
调用例子：`/search/name?query=神秘海域&num=5&page=1`
...
调用例子：`/movies/search/actors?query=小栗旬&num=5&page=1`
...
调用例子：`/movies/search/director?query=姜文&num=5&page=1`
...
调用例子：`/search/category?page=1&num=5&category=动作&area=中国大陆&year=2022`
...
调用例子：`/movies/addPlayCount?id=69785`
...
调用例子：`/movies/addProblem?title=这是一个标题&content=这是一个内容`
...
调用例子：`/movies/problem?page=1&num=5`
...
调用例子：`/movies/addVisitor?address=湖南省&ads=湖南省长沙市`
...
调用例子：`/movies/visitor?page=1&num=5`
...
调用例子：`/user/allUserInfo?num=10&page=1`
